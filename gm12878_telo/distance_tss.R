...
dat$i[(dat$i)=="rep23"] <- "chrM"
dat$i[(dat$i)=="rep24"] <- "chrX"
dat$i[(dat$i)=="rep25"] <- "chrY"
...
ggplot(data =sum_stat, aes(x = i, y = percent_1000, fill = kit))+geom_bar(stat = "identity", position = "dodge")+
  theme_classic()+
  labs(x= "chromosome", y = "Percent reads within 1000bp of TSS")
